...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"js/jquery-1.6.4.js":[function(require,module,exports) {
...
(function (window, undefined) {
  ...
  var jQuery = function () {
    ...
    jQuery.extend({
      ...
      bindReady: function bindReady() {
        ...
        readyList = jQuery._Deferred(); // Catch cases where $(document).ready() is called after the
  ...
  jQuery.extend({
    ...
    Deferred: function Deferred(func) {
      var deferred = jQuery._Deferred(),
          ...
          _promise; // Add errorDeferred methods, then and promise
  ...
  jQuery.support = function () {
    ...
    fragment.appendChild(div.firstChild); // WebKit doesn't clone checked state correctly in fragments

    support.checkClone = fragment.cloneNode(true).cloneNode(true).lastChild.checked;
    div.innerHTML = ""; // Figure out if the W3C box model works as expected

    div.style.width = div.style.paddingLeft = "1px";
    body = document.getElementsByTagName("body")[0]; // We use our own, invisible, body unless the body is already present
    ...
    testElementParent.insertBefore(testElement, testElementParent.firstChild); // Check if a disconnected checkbox will retain its checked
    ...
    testElementParent.removeChild(testElement); // Technique from Juriy Zaytsev
  ...
  jQuery.extend({
    ...
    /* Internal Use Only */
      ...
      if (!cache[id]) {
        cache[id] = {}; // TODO: This is a hack for 1.5 ONLY. Avoids exposing jQuery
      ...
      thisCache = cache[id]; // Internal jQuery data is stored in a separate object inside the object's data
    ...
    /* Internal Use Only */
      ...
      if (name) {
        ...
        if (thisCache) {
          ...
          delete thisCache[name]; // If there is no data left in the cache, we want to continue
      ...
      if (pvt) {
        delete cache[id][internalKey]; // Don't destroy the parent cache unless the internal data object
      ...
      var internalCache = cache[id][internalKey]; // Browsers that fail expando deletion also refuse to delete expandos on
      ...
      if (internalCache) {
        ...
        cache[id][internalKey] = internalCache; // Otherwise, we need to eliminate the expando on the node to avoid
  ...
  jQuery.extend({
    ...
    queue: function queue(elem, type, data) {
      if (elem) {
        type = (type || "fx") + "queue";
        var q = jQuery.data(elem, type, undefined, true); // Speed up dequeue by getting out quickly if this is just a lookup
    ...
    dequeue: function dequeue(elem, type) {
      ...
      var queue = jQuery.queue(elem, type),
          fn = queue.shift(),
          defer; // If the fx queue is dequeued, always remove the progress sentinel
  ...
  jQuery.extend({
      ...
      select: {
        get: function get(elem) {
          ...
          for (var i = one ? index : 0, max = one ? index + 1 : options.length; i < max; i++) {
            var option = options[i]; // Don't return options that are disabled or in a disabled optgroup
    ...
    attr: function attr(elem, name, value, pass) {
      ...
      var ret,
          hooks,
          notxml = nType !== 1 || !jQuery.isXMLDoc(elem); // Normalize the name if needed
      ...
      if (notxml) {
        ...
        if (!hooks) {
          ...
          if (rboolean.test(name)) {
            hooks = boolHook; // Use nodeHook if available( IE6/7 )
          } else if (nodeHook) {
            hooks = nodeHook;
          }
        }
    ...
    removeAttr: function removeAttr(elem, name) {
      ...
      if (elem.nodeType === 1) {
        ...
        elem.removeAttribute(name); // Set corresponding property to false for boolean attributes
  ...
  if (!jQuery.support.getSetAttribute) {
    ...
    nodeHook = jQuery.valHooks.button = {
      get: function get(elem, name) {
        var ret;
        ret = elem.getAttributeNode(name); // Return undefined if nodeValue is empty string
  ...
  jQuery.event = {
    ...
    trigger: function trigger(event, data, elem, onlyHandlers) {
      ...
      event.namespace_re = new RegExp("(^|\\.)" + namespaces.join("\\.(?:.*\\.)?") + "(\\.|$)"); // triggerHandler() and global events don't bubble or run the default action
      ...
      } while (cur && !event.isPropagationStopped()); // If nobody prevented the default action, do it now
    ...
    handle: function handle(event) {
      event = jQuery.event.fix(event || window.event); // Snapshot the handlers list since a called handler may add/remove events.

      var handlers = ((jQuery._data(this, "events") || {})[event.type] || []).slice(0),
          run_all = !event.exclusive && !event.namespace,
          args = Array.prototype.slice.call(arguments, 0); // Use the fix-ed Event rather than the (read-only) native event

      args[0] = event;
      event.currentTarget = this;

      for (var j = 0, l = handlers.length; j < l; j++) {
        var handleObj = handlers[j]; // Triggered event must 1) be non-exclusive and have no namespace, or
  ...
  jQuery.each(["bind", "one"], function (i, name) {
    jQuery.fn[name] = function (type, data, fn) {
      var _handler; // Handle object literals
  ...
  jQuery.fn.extend({
    unbind: function unbind(type, fn) {
      ...
      if (_typeof(type) === "object" && !type.preventDefault) {
        for (var key in type) {
    ...
    toggle: function toggle(fn) {
      ...
      var args = arguments,
        ...
        jQuery.data(this, "lastToggle" + fn.guid, lastToggle + 1); // Make sure that clicks stop
  ...
  jQuery.each(["live", "die"], function (i, name) {
    ...
    /* Internal Use Only */
      ...
      if (_typeof(types) === "object" && !types.preventDefault) {
      ...
      for (j = 0; j < live.length; j++) {
        ...
        if (close.selector === handleObj.selector && (!namespace || namespace.test(handleObj.namespace)) && !close.elem.disabled) {
          ...
          if (handleObj.preType === "mouseenter" || handleObj.preType === "mouseleave") {
            ...
            related = jQuery(event.relatedTarget).closest(handleObj.selector)[0]; // Make sure not to accidentally match a child element with the same selector
  ...
  function cloneCopyEvent(src, dest) {
    ...
    var internalKey = jQuery.expando,
        ...
        curData = jQuery.data(dest, oldData); // Switch to use the internal data object, if it exists, for the next
  ...
  jQuery.buildFragment = function (args, nodes, scripts) {
    var fragment, cacheable, cacheresults, doc; // nodes may contain either an explicit document object,
        ...
        if (typeof elem === "string") {
          if (!rhtml.test(elem)) {
            ...
            var tag = (rtagName.exec(elem) || ["", ""])[1].toLowerCase(),
                ...
                depth = wrap[0],
                div = context.createElement("div"); // Go to html and back, then peel off extra wrappers
  ...
  if (document.documentElement.currentStyle) {
    currentStyle = function currentStyle(elem, name) {
      var left,
          ...
          style = elem.style; // From the awesome hack by Dean Edwards
  ...
  jQuery.extend({
    ...
    ajax: function ajax(url, options) {
      ...
      jqXHR.complete = completeDeferred.done; // Status-dependent callbacks
    ...
    animate: function animate(prop, speed, easing, callback) {
      ...
      return this[optall.queue === false ? "each" : "queue"](function () {
        ...
        var opt = jQuery.extend({}, optall),
            ...
            unit; // will store per property easing and be used to determine when an animation is complete
          ...
          if (isElement && (name === "height" || name === "width")) {
            ...
            if (jQuery.css(this, "display") === "inline" && jQuery.css(this, "float") === "none") {
              if (!jQuery.support.inlineBlockNeedsLayout) {
                ...
                display = defaultDisplay(this.nodeName); // inline-level elements accept inline-block;
    ...
    stop: function stop(clearQueue, gotoEnd) {
      ...
      this.each(function () {
        var timers = jQuery.timers,
            i = timers.length; // clear marker counters if we know they won't be
  ...
  jQuery.extend({
    speed: function speed(_speed, easing, fn) {
      ...
      opt.duration = jQuery.fx.off ? 0 : typeof opt.duration === "number" ? opt.duration : opt.duration in jQuery.fx.speeds ? jQuery.fx.speeds[opt.duration] : jQuery.fx.speeds._default; // Queueing
  ...
  jQuery.offset = {
    initialize: function initialize() {
      ...
      checkDiv.style.top = "20px"; // safari subtracts parent border width here which is 5px
    ...
    jQuery.fn[type] = function (size) {
      ...
      if (jQuery.isWindow(elem)) {
        ...
        var orig = jQuery.css(elem, type),
            ret = parseFloat(orig);
        return jQuery.isNaN(ret) ? orig : ret; // Set the width or height on the element (default to pixels if value is unitless)
      } else {
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","js/jquery-1.6.4.js"], null)
//# sourceMappingURL=/jquery-1.6.4.2085085f.js.map